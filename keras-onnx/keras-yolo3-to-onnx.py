...
class YOLOEvaluationLayer(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        yolo_outputs = inputs[0:2]
        input_image_shape = K.squeeze(inputs[2], axis=0)
...
class YOLO(object):
    def __init__(self,model_path,classes_path,anchor_path):
    ...
    def load_model(self):
        ...
        try:
            ...
            self.yolo_model = tiny_yolo_body(image_input, num_anchors // 2, num_classes) \
...
if __name__ == '__main__':
    if len(sys.argv) < 5:
        print("python keras-yolo3-to-onnx.oy input.h5 classes.txt anchors.txt output.onnx\n")
        exit(-1)

    onnx_model_file_name = sys.argv[4]

    target_opset = 10

    yolo = YOLO(sys.argv[1],sys.argv[2],sys.argv[3])

    onnxmodel = convert_model(yolo, onnx_model_file_name, target_opset)

    print("success to output "+onnx_model_file_name)

    #detect_img(yolo, "../images/person.jpg", model_file_name)
